...
serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: appointment, error: apptError } = await serviceClient
      .from('appointments')
      .select('id, trainer_id, trainee_id, title, proposed_slots')
      .eq('id', appointment_id)
      .single();

    if (apptError || !appointment) return json({ error: 'Appointment not found' }, 404);
    if (appointment.trainer_id !== user.id) return json({ error: 'Forbidden' }, 403);

    const { data: tokens, error: tokenError } = await serviceClient
      .from('device_tokens')
      .select('token')
      .eq('user_id', appointment.trainee_id)
      .eq('enabled', true)
      .is('revoked_at', null);
    ...
    console.error('notify-appointment-proposal error:', err);
