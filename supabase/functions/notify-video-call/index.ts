...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!payload.call_id) {
      return json({ error: 'Missing call_id' }, 400);
    }
    ...
    const { data: call, error: callError } = await serviceClient
      .from('video_calls')
      .select('id, channel_id, agora_channel_name, started_by, ended_at')
    ...
    if (call.ended_at) {
      return json({ sent: 0, skipped: 'call_ended' });
    }
    ...
    console.error('notify-video-call error:', err);
