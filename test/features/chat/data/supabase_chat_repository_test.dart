...
class _FakeChatRepository implements ChatRepository {
  ...
  @override Future<void> sendMessage(String c, String m) async {}
  ...
  @override Stream<Set<String>> watchTypingUsers(String c) => const Stream.empty();
}

void main() {
  group('ChatChannel — domain logic', () {
    test('isModerator true for moderator role', () {
      final ch = ChatChannel(id:'1',type:ChannelType.direct,createdAt:DateTime(2026),
        currentUserRole:MemberRole.moderator,unreadCount:0);
      expect(ch.isModerator, isTrue);
    });
    test('isModerator false for member role', () {
      final ch = ChatChannel(id:'1',type:ChannelType.direct,createdAt:DateTime(2026),
        currentUserRole:MemberRole.member,unreadCount:0);
    ...
    test('channelDisplayName returns packageName for community', () {
      final ch = ChatChannel(id:'1',type:ChannelType.community,packageId:'moro',
        createdAt:DateTime(2026),currentUserRole:MemberRole.member,unreadCount:0);
      expect(ch.channelDisplayName(packageName:'Moro'), 'Moro');
    });
    test('fromJson parses correctly', () {
      final json = {'id':'abc','type':'community','package_id':'moro','created_at':'2026-04-13T10:00:00.000Z'};
  ...
  group('ChatMessage — domain logic', () {
    test('isDeleted true when deletedAt set', () {
      final msg = ChatMessage(id:'1',channelId:'c',senderId:'u',content:'hi',
        isBotResponse:false,isCallRequest:false,deletedAt:DateTime(2026),createdAt:DateTime(2026));
      expect(msg.isDeleted, isTrue);
    });
    test('isDeleted false when deletedAt null', () {
      final msg = ChatMessage(id:'1',channelId:'c',senderId:'u',content:'hi',
        isBotResponse:false,isCallRequest:false,createdAt:DateTime(2026));
      expect(msg.isDeleted, isFalse);
    });
    test('isOwnMessage matches senderId', () {
      final msg = ChatMessage(id:'1',channelId:'c',senderId:'user-abc',content:'hi',
        isBotResponse:false,isCallRequest:false,createdAt:DateTime(2026));
    ...
    test('fromJson parses all fields', () {
      final json = {
        'id':'m1','channel_id':'c1','sender_id':'u1','content':'hello',
        'is_bot_response':true,'is_call_request':false,'deleted_at':null,
  ...
  group('FakeChatRepository — interface compliance', () {
    ...
    test('sendMessage completes', () async {
      await expectLater(_FakeChatRepository().sendMessage('c','hi'), completes);
    });
  });
}
